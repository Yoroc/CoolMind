"""Final demonstration of CoolMind's quantization capabilities"""
...
def demo_quantization_with_monitoring():
    """Demo quantization with active thermal monitoring using proper mocking"""
    print("=== CoolMind Quantization Demo with Monitoring ===\n")
    
    # Create engine with monitoring enabled and thresholds suitable for demo
    config = ThermalConfig(
        cooldown_threshold=30.0,   # Lower thresholds for demo visibility
    ...
    # Test different temperatures by properly mocking the WMI interface
    ...
    for i, temp in enumerate(test_temps):
        ...
        # Create a proper mock for WMI
        with patch('coolmind.core.engine.wmi') as mock_wmi:
            # Setup the mock to return our temperature
            ...
            # Give the monitoring thread a moment to update (2x update interval)
            time.sleep(config.update_interval * 2.1)
